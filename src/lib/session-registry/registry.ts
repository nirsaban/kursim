...
export async function evictSession(sid: string, opts: { notify?: boolean } = {}): Promise<void> {
  ...
  const multi = r.multi().del(sessKey(sid));
...
/**
 * Rotate the refresh token. A presented token that does not match the stored
 * hash means the token was already rotated — assume theft and kill the session.
 */
export async function rotateRefreshToken(sid: string, presented: string): Promise<RotateResult> {
  ...
  const valid = await verifyPassword(stored, presented);
  if (!valid) {
  ...
  const refreshToken = randomBytes(32).toString('base64url');
  const refreshHash = await hashPassword(refreshToken);
  await r.hset(sessKey(sid), { refreshHash });
