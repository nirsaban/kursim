    ...
    >
        ...
        >
          <div>
            <Reveal>
              ...
              {m.subheadline && (
                <p className="text-lg text-paper/70 mt-5 max-w-2xl leading-relaxed">
                  {m.subheadline}
                </p>
              )}
            </Reveal>
            <Reveal delay={150}>
              <div className="flex flex-wrap items-center gap-4 mt-8">
                ...
                {!heroMedia && m.instructorName && (
                  <span className="text-sm text-paper/70">
                    {he.withInstructor} <b className="text-paper">{m.instructorName}</b>
                  </span>
                )}
              </div>
              ...
              <a
                href={`/t/${slug}/redeem`}
                className="inline-flex items-center gap-1.5 mt-4 text-sm font-semibold text-paper/70 hover:text-paper underline underline-offset-4 decoration-dotted"
              >
                <span aria-hidden>🎟️</span>
                {he.landingHaveCode}
              </a>
              {enrollCount >= 3 && (
                <p className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-paper">
                  <span className="flex -space-x-2" aria-hidden>
                    {['👩', '🧑', '👨'].map((e, i) => (
                      <span
                        key={i}
                        className="w-7 h-7 rounded-full grid place-items-center text-sm border-2 border-ink"
              ...
              <div className="flex flex-wrap items-stretch gap-x-6 gap-y-3 mt-8 text-[13px] text-paper/70">
                {(
                  [
                    modules.length > 0 && {
                      num: modules.length,
                  ...
                  .map((s, i) => (
                    <div key={s.label} className="flex items-stretch gap-6">
                      {i > 0 && <div className="w-px bg-paper/15" aria-hidden />}
                      <div>
                        <CountUp
                          value={s.num}
                          decimals={s.decimals}
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {m.benefits.map((b, i) => (
              <Reveal key={i} delay={i * 80} className="h-full">
                <div
                  className="border border-line rounded-xl2 p-6 h-full bg-card transition-colors duration-300 hover:border-[--hover-line]"
                  style={{ '--hover-line': theme.main } as React.CSSProperties}
                >
                  <span
                    className="inline-flex w-11 h-11 rounded-full items-center justify-center font-display font-bold text-lg"
            ...
            {galleryRest.map((item, i) => (
              <Reveal key={i} delay={i * 80}>
                <figure>
                  ...
                  {item.caption && (
                    <figcaption className="text-sm text-muted mt-2 px-1">{item.caption}</figcaption>
                  )}
                </figure>
              </Reveal>
            ))}
          ...
          <div className="relative max-w-3xl mx-auto px-4 py-16">
            <Reveal>
              ...
              <p className="text-sm text-brand-300 mb-7">
                <span dir="ltr">{modules.length}</span> {he.modules} ·{' '}
                ...
                <Reveal key={mod.id} delay={mi * 60}>
                  ...
                  >
                    <summary className="flex items-center gap-4 px-5 py-4 cursor-pointer select-none list-none [&::-webkit-details-marker]:hidden">
                      ...
                      <span className="text-brand-300 transition-transform group-open:rotate-180" aria-hidden>
                        ▾
                      </span>
                    </summary>
                    <ul className="border-t border-paper/10 px-5 py-2.5 space-y-1">
                      {mod.lessons.map((l) => (
                        <li key={l.id} className="flex items-center gap-3 py-1.5 text-sm">
                          ...
                          {l.durationSec ? (
                            <span className="text-xs tabular-nums font-mono text-brand-300" dir="ltr">
          ...
          <Reveal>
            <div className="flex items-start gap-5">
              ...
              <div>
                ...
                {m.aboutSchool && (
                  <p className="text-muted mt-3 leading-relaxed whitespace-pre-wrap">{m.aboutSchool}</p>
                )}
              </div>
            </div>
          </Reveal>
      ...
      {showSale && (
        <section id="sale" className="max-w-5xl mx-auto px-4 py-16">
          <Reveal>
            <div
              className="rounded-xl2 border-2 px-6 py-8 sm:px-10"
                ...
                {m.faq.map((f, i) => (
                  <Reveal key={i} delay={i * 50}>
                    <details className="group bg-card border border-line rounded-[13px]">
                      ...
                      <p className="px-5 pb-5 text-sm text-muted leading-relaxed whitespace-pre-wrap">{f.a}</p>
                    </details>
                  </Reveal>
                ))}
              </div>
